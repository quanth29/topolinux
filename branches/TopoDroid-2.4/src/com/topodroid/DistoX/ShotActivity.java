                        ...
                        , INewPlot
{
  ...
  void setTheTitle()
  {
    ...
    // FIXME setTitleColor( TopoDroidApp.COLOR_NORMAL );
  }
  ...
  public void onItemClick(AdapterView<?> parent, View view, int pos, long id)
  {
    ...
    if ( mMenu == (ListView)parent ) {
      ...
      } else if ( TopoDroidApp.mLevelOverBasic && p++ == pos ) { // RECOVER
        (new DistoXUndelete(this, this, mApp.mData, mApp.mSID ) ).show();
        updateDisplay( );
      } else if ( TopoDroidApp.mLevelOverNormal && p++ == pos ) { // PHOTO
        startActivity( new Intent( this, PhotoActivity.class ) );
      } else if ( TopoDroidApp.mLevelOverNormal && p++ == pos ) { // SENSORS
        startActivity( new Intent( this, SensorListActivity.class ) );
      } else if ( TopoDroidApp.mLevelOverBasic && p++ == pos ) { // 3D
      ...
      } else if ( TopoDroidApp.mLevelOverNormal && p++ == pos ) { // DEVICE
...
//       mButton1[0].setBackgroundResource( R.drawable.ic_download_on );
//     } else {
//       setTitleColor( TopoDroidApp.COLOR_NORMAL );
//       mButton1[0].setBackgroundResource( R.drawable.ic_download );
//     }
//   }
// 
//   // FIXME_COMM
...
//       if ( mConnected == false ) {
//         mConnected = mApp.connect( mApp.mDevice.mAddress, this );
//       } else {
//         mApp.disconnect( );
//         mConnected = false;
//       }
...
// ---------------------------------------------------------------
  ...
  public void onCreate(Bundle savedInstanceState)
  {
    ...
    icons00no = ( TopoDroidApp.mSizeButtons == 2 )? ixonsno : iconsno;

    mNrButton1 = TopoDroidApp.mLevelOverBasic ? 7 : 5;
    // mNrButton2 = 7;
    ...
    mDataDownloader.mButtonDownload = mButton1[0]; // download-index
    ...
    mImage.setBackgroundResource( 
      ( TopoDroidApp.mSizeButtons == 2 )? R.drawable.ix_menu : R.drawable.ic_menu );
  ...
  public void onClick(View view)
  {
    ...
    if ( b != null ) {
      ...
      } else if ( k1 < mNrButton1 && b == mButton1[k1++] ) { // mBtnSplays 
  ...
  public void startExistingPlot( String name, long type ) // name = plot/sketch3d name
  {
    ...
    Log.v( TopoDroidApp.TAG, "startExistingPlot \"" + name + "\" type " + type + " sid " + mApp.mSID );
  ...
  */
  ...
  private void setMenuAdapter()
  {
    ...
    if ( TopoDroidApp.mLevelOverBasic  ) mMenuAdapter.add( res.getString( menus[1] ) );
    if ( TopoDroidApp.mLevelOverNormal ) mMenuAdapter.add( res.getString( menus[2] ) );
    if ( TopoDroidApp.mLevelOverNormal ) mMenuAdapter.add( res.getString( menus[3] ) );
    if ( TopoDroidApp.mLevelOverBasic  ) mMenuAdapter.add( res.getString( menus[4] ) );
    if ( TopoDroidApp.mLevelOverNormal ) mMenuAdapter.add( res.getString( menus[5] ) );
