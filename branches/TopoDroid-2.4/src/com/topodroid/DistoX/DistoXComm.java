...
// Bluetooth receiver
  ...
  {
    // TopoDroidApp.Log(  TopoDroidApp.LOG_BT, "resetBTReceiver");
    if ( mBTReceiver != null ) {
...
************* */
  ...
  private class RfcommThread extends Thread
  {
    ...
    public void run()
    {
      ...
      while ( doWork && nReadPackets != toRead ) {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "RFcomm readPacket returns " + res );
        if ( res == DistoXProtocol.DISTOX_PACKET_NONE ) {
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "RFcomm readPacket returns ERR_OFF " );
          doWork = false;
  ...
  private void createSocket( String address )
  {
    ...
    if ( mProtocol == null || ! address.equals( mAddress ) ) {
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "createSocket() device " + mBTDevice.getName() );
      try {
        if ( mBTSocket != null ) {
  ...
  private boolean sendCommand( int cmd )
  {
    ...
    for (int k=0; k<3 && ! ret; ++k ) { // try three times
      ...
      TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "sendCommand " + cmd + " " + k + "-ret " + ret );
      try {
        Thread.sleep( 100 );
      } catch ( InterruptedException e ) {
      }
  ...
  private boolean setX310CalibMode( boolean turn_on )
  {
    ...
    TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "setX310CalibMode ret " + ret );
    return ret;
  }
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "toggleCalibMode address " + address + " type " + type );
    if ( ! checkRfcommThreadNull( "toggleCalibMode address " + address ) ) {
      TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "toggleCalibMode address " + address + " not null RFcomm thread" );
      return false;
    }
    boolean ret = false;
    if ( connectSocket( address ) ) {
      ...
      switch ( type ) {
        case Device.DISTO_A3:
          ...
          if ( ! mProtocol.read8000( result ) ) { // FIXME ASYNC
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "toggleCalibMode A3 failed read8000" );
            destroySocket( true );
            return false;
          }
          TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "toggleCalibMode A3 result " + result[0] );
        ...
        case Device.DISTO_X310:
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "toggleCalibMode X310 setX310CalibMode " + mCalibMode );
    ...
    }
    {
      ...
      if ( connectSocket( address ) ) {
        ...
        if ( mApp.distoType() == Device.DISTO_A3 ) {
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "downloadData A3 to-read " + to_read );
          if ( to_read <= 0 ) {
            destroySocket( true );
            return to_read;
          }
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "downloadData A3 read " + nReadPackets );
          destroySocket( true );
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_COMM, "DistoXComm.downloadData X310 read " + nReadPackets );
          destroySocket( true );
        } else {
          TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "DistoXComm.downloadData unknown DistoType " + mApp.distoType() );
        }
        if ( mRfcommThread != null ) {
          try {
            mRfcommThread.join();
