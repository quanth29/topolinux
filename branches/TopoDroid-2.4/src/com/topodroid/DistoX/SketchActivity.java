...
*/  
  ...
  private class SaveTh3File extends AsyncTask<Intent,Void,Boolean>
  {
    ...
    {
      try {
        String filename = mApp.getTh3FileWithExt( mFullName );
  ...
  public boolean onTouch( View view, MotionEvent rawEvent )
  {
    ...
    float d0 = TopoDroidApp.mCloseCutoff + TopoDroidApp.mCloseness / mInfo.zoom_3d;
  ...
  // CLICK
	...
	return;
      ...
      if ( ( b == mButton2[0] && mMode == SketchDef.MODE_DRAW ) || 
        ...
        int split = mModel.refineToMaxSide( TopoDroidApp.mSketchSideSize );
        if ( split == 0 ) { 
    ...
    {
      // Log.v("DistoX", "set compass heading to " + (int)heading );
      // if ( mSectionType == SketchSection.SECTION_VERT ) {
        mInfo.resetDirection( heading, 0.0f ); // from the side
      // } else { // mSectionType == SketchSection.SECTION_HORIZ
      //   mInfo.resetDirection( heading, 90.0f ); // from above
      // }
      mCompass = null;
      setMode( SketchDef.MODE_MOVE );
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
      ...
      if (item == mMIdelete ) {
        ...
        mCompass = new SketchCompassSensor( this, sm, TopoDroidApp.mCompassReadings );
