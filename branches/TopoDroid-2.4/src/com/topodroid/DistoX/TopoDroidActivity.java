                               ...
                               , OnDismissListener
{
    ...
    if ( mMenu == (ListView)parent ) {
      ...
      if ( p++ == pos ) { // PALETTE
        ...
        if ( p++ == pos ) { // SETTINGS
          ...
          if ( TopoDroidApp.mLevelOverAdvanced && p++ == pos ) { // LOGS
            intent = new Intent( this, TopoDroidPreferences.class );
            intent.putExtra( TopoDroidPreferences.PREF_CATEGORY, TopoDroidPreferences.PREF_CATEGORY_LOG );
            startActivity( intent );
          } else {  
            if ( TopoDroidApp.mLevelOverAdvanced && mApp.mCosurvey && p++ == pos ) {  // CO-SURVEY
  ...
  private class ImportPocketTopoTask extends AsyncTask< String, Integer, Long >
  {
    ...
    protected Long doInBackground( String... str )
    {
      ...
      try {
        ...
        long id = mApp.mData.insertShots( sid, 1, shots ); // start id = 1
        TopoDroidApp.Log( TopoDroidApp.LOG_PTOPO, "SID " + sid + " inserted shots. return " + id );
        ...
        if ( blk != null && plan != null || extended != null ) {
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_PTOPO, "SID " + sid + " scraps " + plan.length() + " " + extended.length() );
          try {
            String filename1 = mApp.getTh2File( parser.mName + "-1p.th2" );
            FileWriter fw1 = new FileWriter( filename1 );
            PrintWriter pw1 = new PrintWriter( fw1 );
            pw1.format("%s", plan );
            
            String filename2 = mApp.getTh2File( parser.mName + "-1s.th2" );
            FileWriter fw2 = new FileWriter( filename2 );
            PrintWriter pw2 = new PrintWriter( fw2 );
            pw2.format("%s", extended );
          } catch ( IOException e ) {
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "SID " + sid + " scraps IO error " + e );
          }
       }
      } catch ( ParserException e ) {
        // Toast.makeText(this, R.string.file_parse_fail, Toast.LENGTH_SHORT).show();
  ...
  void setMenuAdapter()
  {
    ...
    if ( TopoDroidApp.mLevelOverAdvanced ) mMenuAdapter.add( res.getString( menus[2] ) );
    if ( TopoDroidApp.mLevelOverAdvanced && mApp.mCosurvey ) mMenuAdapter.add( res.getString( menus[3] ) );
  ...
  void resetButtonBar()
  {
    ...
    mNrButton1 = 3 + ( TopoDroidApp.mLevelOverAdvanced ? 2 : 0 );
    // int nr_button2 = 3;
    mButton1 = new Button[mNrButton1];
    // mButton2 = new Button[nr_button2];

    mImage.setBackgroundResource( 
      ( TopoDroidApp.mSizeButtons == 2 )? R.drawable.ix_menu
                                       : R.drawable.ic_menu );
  ...
  //   // // TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "scale " + TopoDroidApp.mSizeButtons );
  //   // icons00 = ( TopoDroidApp.mSizeButtons == 2 )? ixons : icons;
