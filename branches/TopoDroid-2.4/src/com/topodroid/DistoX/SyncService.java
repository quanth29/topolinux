...
public class SyncService 
{
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync connect to " + device.getName() );
  ...
  private synchronized void reconnect()
  {
    ...
    TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync reconnect to " + mRemoteDevice.getName() );
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync connected. remote device " + device.getName() );
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync disconnect");
    if ( mConnectingThread != null ) { mConnectingThread.cancel();   mConnectingThread   = null; }
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "sync connection failed");
    mRemoteDevice = null;
    setConnectState(STATE_NONE);
    mType = STATE_NONE;
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "sync connection lost");
    mRemoteDevice = null;
    setConnectState(STATE_NONE);
  ...
  private class AcceptThread extends Thread 
  {
    ...
    private void createServerSocket()
    {
      ...
      try { // Create a new listening server socket
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "listen() failed " + e);
      }
      mmServerSocket = tmp;
    }
    ...
    public void run()
    {
      ...
      while ( mAcceptRun && mAcceptState == STATE_LISTEN ) {
        try {
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "accept() failed " + e);
          break;
        }

        if (socket != null) { // If a connection was accepted
          TopoDroidApp.Log(TopoDroidApp.LOG_SYNC, "incoming connection request " + socket.getRemoteDevice().getName() );
            ...
            switch ( mConnectState ) {
              ...
              case STATE_CONNECTED: // Either not ready or already connected. Terminate new socket.
                try {
                  ...
                  TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "Could not close unwanted socket " + e);
                }
                break;
            }
          }
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync AcceptThread done");
    }
    ...
    public void cancel()
    {
      try {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "close() of server failed " + e);
      }
    }
  }
  ...
  private class ConnectingThread extends Thread
  {
    ...
    public ConnectingThread( BluetoothDevice device )
    {
      ...
      try {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "ConnectingThread cstr failed " + e);
      }
      mmSocket = tmp;
    }
    ...
    public void run()
    {
      ...
      try { // Make a connection to the BluetoothSocket
        ...
        try { // Close the socket
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "unable to close() socket during connection failure " + e2);
        }
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync connecting thread done");
    }
    ...
    public void cancel()
    {
      try {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "close() of connect socket failed " + e);
      }
    }
  }
  ...
  private class ConnectedThread extends Thread
  {
    ...
    public ConnectedThread(BluetoothSocket socket)
    {
      ...
      try { // Get the BluetoothSocket input and output streams
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "temp sockets not created " + e);
      }
    ...
    public void run() 
    {
      ...
      while ( mConnectRun ) { // Keep listening to the InputStream while connected
        try {
          ...
          TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "disconnected " + e);
          try { // Close the socket
            mmSocket.close();
          } catch (IOException e2) {
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "unable to close() socket during connection failure " + e2);
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "sync ConnectedThread done type " + mType );
    ...
    public void write( byte[] buffer ) 
    {
      ...
      try {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "Exception during write " + e );
      }
    }
