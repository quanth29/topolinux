...
class DistoXNum
{
  ...
  // public void dump()
  // {
  //   TopoDroidApp.Log( TopoDroiaApp.LOG_NUM, "DistoXNum Stations:" );
  //   for ( NumStation st : mStations ) {
  //     TopoDroidApp.Log( TopoDroiaApp.LOG_NUM, "   " + st.name + " S: " + st.s + " E: " + st.e );
  //   }
  //   TopoDroidApp.Log( TopoDroiaApp.LOG_NUM, "Shots:" );
  //   for ( NumShot sh : mShots ) {
  //     TopoDroidApp.Log( TopoDroiaApp.LOG_NUM, "   From: " + sh.from.name + " To: " + sh.to.name );
  //   }
  // } 
  ...
  {
    if ( ts.reversed != 1 ) {
      TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "making shot from reversed temp " + sf.name + " " + st.name );
    }
  ...
  private boolean computeNum( List<DistoXDBlock> data, String start, String[] barrier )
  {
    ...
    for ( DistoXDBlock blk : data ) {
      switch ( blk.type() ) {
        ...
        case DistoXDBlock.BLOCK_BLANK:
          if (lastLeg != null ) {
            if ( blk.relativeDistance( lastLeg.getFirstBlock() ) < TopoDroidApp.mCloseDistance ) {
    ...
    // TopoDroidApp.Log( TopoDroiaApp.LOG_NUM,
    ...
    // TopoDroidApp.Log( TopoDroiaApp.LOG_NUM, "DistoXNum::compute done leg shots ");
  ...
  private void makeSingleLoops( ArrayList<NumBranch> branches, ArrayList<NumShot> shots )
  {
    for ( NumShot shot : shots ) {
      ...
      sh0.mBranchDir = 1;
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "Start branch from station " + sf0.name );
  ...
  private void compensateSingleLoops( ArrayList<NumBranch> branches )
  {
    for ( NumBranch br : branches ) {
      br.computeError();
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "Single loop branch error " + br.e + " " + br.s + " " + br.v );
  ...
  void doLoopCompensation( ArrayList< NumNode > nodes, ArrayList< NumShot > shots )
  {
    ...
    for ( NumCycle c : cycles ) {
      c.computeError();
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "cycle error " + c.e + " " + c.s + " " + c.v ); 
    }
    ...
    //   }
    //   TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "AA " + sb.toString() );
    // }
    ...
    //     sb.append( Float.toString(aa[ y1*ls + y2]) ).append("  ");
    //   }
    //   TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "invAA " + sb.toString() );
    // }
    ...
    for (int y=0; y<ls; ++y ) { // compute the closure compensation values
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "cycle correction " + cy.ce + " " + cy.cs + " " + cy.cv ); 
    }
