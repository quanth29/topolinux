...
public class DataHelper extends DataSetObservable
{
  ...
  static final String DB_VERSION = "19";
...
//these are real database "delete"
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_DB, "updateShotExtend <" + id + "> " + extend );
    // if ( myDB == null ) return;
  ...
  {
    // if ( myDB == null ) return;
    // TopoDroidApp.Log( TopoDroidApp.LOG_DB, "deleteShot: " + id + "/" + sid );
  ...
  {
    // if ( myDB == null ) return;
    // TopoDroidApp.Log( TopoDroidApp.LOG_DB, "undeleteShot: " + id + "/" + sid );
  ...
  public long insertShots( long sid, long id, ArrayList< ParserShot > shots )
  {
    ...
    try {
      ...
      for ( ParserShot s : shots ) {
        ...
        ih.execute();
        // TopoDroidApp.Log( TopoDroidApp.LOG_DEBUG, "shot " + id + ": " + s.from + "-" + s.to );
        ++id;
      }
      myDB.setTransactionSuccessful();
    } finally {
  ...
  {
    TopoDroidApp.Log( TopoDroidApp.LOG_DB, "insertShot <" + id + "> " + from + "-" + to + " extend " + extend );
    // if ( myDB == null ) return -1;
    if ( id == -1L ) {
      ++ myNextId;
      id = myNextId;
...
*/
   ...
   public List< PlotInfo > selectAllPlots( long sid, long status )
   {
     ...
     // if ( myDB == null ) return list;
     Cursor cursor = myDB.query(PLOT_TABLE,
			        new String[] { "id", "name", "type", "start", "view", "xoffset", "yoffset", "zoom", "azimuth" }, // columns
                                "surveyId=? and status=?", 
                                new String[] { Long.toString(sid), Long.toString(status) }, 
                                null,  // groupBy
                                null,  // having
   ...
   {
     // TopoDroidApp.Log( TopoDroidApp.LOG_DB, "selectShot " + id + "/" + sid );
     // if ( myDB == null ) return null;
   ...
   public void selectCalibError( long cid, CalibResult res )
   {
     ...
     if (cursor.moveToFirst()) {
       ...
       try {
         ...
         TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "selectCalibError parse Float error: calib ID " + cid );
       }
     }
     if (cursor != null && !cursor.isClosed()) {
       cursor.close();
   ...
   private List<String> selectAllNames( String table )
   {
     ...
     TopoDroidApp.Log( TopoDroidApp.LOG_DB, "found " + list.size() + " names " );
     return list;
   }
   ...
   public void setValue( String key, String value )
   {
     if ( key == null || key.length() == 0 ) {
       TopoDroidApp.Log( TopoDroidApp.LOG_DB, "DataHelper::setValue null key");
       return;
     }
     if ( value == null || value.length() == 0 ) {
       TopoDroidApp.Log( TopoDroidApp.LOG_DB, "DataHelper::setValue null value");
       return;
     }
     // if ( myDB == null ) return;
     Cursor cursor = myDB.query( CONFIG_TABLE,
         ...
         do {
           ...
           if ( name.startsWith( prefix ) /* && ( type == PlotInfo.PLOT_PHOTO || type == PlotInfo.PLOT_SECTION ) */ ) {
             try {
               ...
               TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "getNextSectionId parse Int error: survey ID " + sid );
             }
           }
         } while (cursor.moveToNext());
       }
   ...
   public PlotInfo getPlotInfo( long sid, String name )
   {
     ...
     if ( myDB != null && name != null ) {
       Cursor cursor = myDB.query( PLOT_TABLE, 
                 new String[] { "id", "type", "start", "view", "xoffset", "yoffset", "zoom", "azimuth" },
                 "surveyId=? and name=?", 
                 new String[] { Long.toString(sid), name },
                 null, null, null );
       if (cursor != null ) {
   ...
   public long insertPlot( long sid, long id, String name, long type, long status, String start, String view,
                           double xoffset, double yoffset, double zoom, double azimuth, boolean forwrad )
   {
     ...
     if ( forwrad ) {
       ...
       // synchronized( mListeners )
       for ( DataListener listener : mListeners ) {
         listener.onInsertPlot( sid, id, name, type, status, start, view, xoffset, yoffset, zoom, azimuth );
       }
     }
     return id;
   }
  ...
  public boolean updateFixedStation( long id, long sid, String station )
  {
   ...
   {
     // TopoDroidApp.Log( TopoDroidApp.LOG_DB, "updateCalibAlgo id " + id + " algo " + algo );
   ...
   {
     // TopoDroidApp.Log( TopoDroidApp.LOG_DB, "updateCalibCoeff id " + id + " coeff. " + coeff );
   ...
   public long setSurvey( String name, boolean forward )
   {
     ...
     Log.v( TopoDroidApp.TAG, "setSurvey " + name + " forward " + forward + " listeners " + mListeners.size() );
     if ( forward ) {
       // synchronized( mListeners )
       for ( DataListener listener : mListeners ) {
   ...
   public void dumpToFile( String filename, long sid )
   {
     ...
     try {
       ...
       cursor = myDB.query( PLOT_TABLE, 
                            new String[] { "id", "name", "type", "status", "start", "view", "xoffset", "yoffset", "zoom", "azimuth" },
                            "surveyId=?", new String[] { Long.toString( sid ) },
                            null, null, null );
       if (cursor.moveToFirst()) {
         do {
           pw.format(Locale.ENGLISH,
                     "INSERT into %s values( %d, %d, \"%s\", %d, %d, \"%s\", \"%s\", %.2f, %.2f, %.2f, %.2f );\n",
                     PLOT_TABLE,
                     sid,
                     cursor.getLong(0),
                     cursor.getString(1),
   ...
   private long longValue( String val )
   {
     ...
     if ( ! toParse.equals("\"null\"") ) {
       try {
         ...
         TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "longValue error: " + val.substring( pos, next_pos ) );
       }
     }
     pos = next_pos;
     skipCommaAndSpaces( val );
   ...
   private double doubleValue( String val )
   {
     ...
     try {
       ...
       TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "doubleValue error: " + val.substring(pos, next_pos) );
     }
     pos = next_pos;
     skipCommaAndSpaces( val );
     return ret;
       ...
       if ( table.equals(SURVEY_TABLE) ) {
         ...
         while ( (line = br.readLine()) != null ) {
           ...
           if ( table.equals(PHOTO_TABLE) ) {
             ...
             insertPlot( sid, id, name, type, status, start, view, xoffset, yoffset, zoom, azimuth, false );
   ...
   private static class DistoXOpenHelper extends SQLiteOpenHelper
   {
      ...
      {
        Log.v( TopoDroidApp.TAG, "DistoXOpenHelper onCreate db ");
        createTables( db );
        // TopoDroidApp.Log( TopoDroidApp.LOG_DB, "DistoXOpenHelper onCreate done db " + db );
      }
