...
class PTFile
{
  ...
  private static void write( FileOutputStream fs, byte[] b, int n )
  {
    try {
      ...
      TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "IO error on write " + n + " bytes: " + e.toString() );
    }
  }
  ...
  private static void read( FileInputStream fs, byte[] b, int n )
  {
    try {
      ...
      TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "IO error on read " + n + " bytes: " + e.toString() );
    }
  }
  ...
  static float ROLL_2_DEG( float r )
  {
    ...
    void read( FileInputStream fs )
    {
      ...
      try {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "IO error on \"Top3\"");
      }
...
*/
    ...
    int addShot( short trip, String from, String to, 
                 ...
                 String comment )
    {
      // Log.v( "DistoX", "PT file add shot " + from + " " + to + " " + distance + " " + azimuth + " " + inclination );
