...
// ---------------------------------------------------------------
  ...
  public void onCreate(Bundle savedInstanceState)
  {
    ...
    if ( mApp.mDevice == null ) {
      mButton1[0].setBackgroundResource( icons[9] ); // ic_add
    }
    ...
    if ( from != null && to != null && from.length() > 0 ) {
      ...
      // } else
      {
        ...
        if ( left != null && left.length() > 0 ) {
          float l = -1.0f;
          try {
            l = Float.parseFloat( left ) / TopoDroidApp.mUnitLength;
          } catch ( NumberFormatException e ) {
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "parse Float error: left " + left );
          }
          if ( l >= 0.0f ) {
            if ( horizontal ) {
              if ( at >= 0L ) {
        ...
        if ( right != null && right.length() > 0 ) {
          float r = -1.0f;
          try {
            r = Float.parseFloat( right ) / TopoDroidApp.mUnitLength;
          } catch ( NumberFormatException e ) {
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "parse Float error: right " + right );
          }
          if ( r >= 0.0f ) {
            if ( horizontal ) {
              if ( at >= 0L ) {
        ...
        if ( up != null && up.length() > 0 ) {
          float u = -1.0f;
          try {
            u = Float.parseFloat( up ) / TopoDroidApp.mUnitLength;
          } catch ( NumberFormatException e ) {
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "parse Float error: up " + up );
          }
          if ( u >= 0.0f ) {
            if ( horizontal ) {
              if ( at >= 0L ) {
        ...
        if ( down != null && down.length() > 0 ) {
          float d = -1.0f;
          try {
            d = Float.parseFloat( down ) / TopoDroidApp.mUnitLength;
          } catch ( NumberFormatException e ) {
            TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "parse Float error: down " + down );
          }
          if ( d >= 0.0f ) {
            if ( horizontal ) {
              if ( at >= 0L ) {
...
*/
  ...
  public boolean onCreateOptionsMenu(Menu menu) 
  {
    ...
    mMIsurvey.setIcon( R.drawable.ic_meta );
