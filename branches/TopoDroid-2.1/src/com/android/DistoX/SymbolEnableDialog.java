                         ...
                         implements View.OnClickListener
{
  ...
  boolean createAdapters()
  {
    ...
    for ( int i=0; i<np; ++i ) {
      mPointAdapter.add( new EnableSymbol( mParent, this, DrawingActivity.SYMBOL_POINT, i, point_lib.getAnyPoint( i ) ) );
    }
    for ( int j=0; j<nl; ++j ) {
      mLineAdapter.add( new EnableSymbol( mParent, this, DrawingActivity.SYMBOL_LINE, j, line_lib.getAnyLine( j ) ) );
    }
    for ( int k=0; k<na; ++k ) {
      mAreaAdapter.add( new EnableSymbol( mParent, this, DrawingActivity.SYMBOL_AREA, k, area_lib.getAnyArea( k ) ) );
    }
