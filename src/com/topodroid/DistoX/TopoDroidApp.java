                          ...
                          implements OnSharedPreferenceChangeListener
{
      ...
      return dir.listFiles( new FileFilter() {
          public boolean accept( File pathname ) { 
            ...
            return (ext1 == null && ext2 == null)
                  || ( ext1 != null && pathname.getName().endsWith( ext1 ) )
                  || ( ext2 != null && pathname.getName().endsWith( ext2 ) );
          }
        } );
    }
    return null;
  ...
  public static File[] getZipFiles() { return getFiles( APP_ZIP_PATH, ".zip", null ); }
  public static File[] getBinFiles() { return getFiles( APP_BIN_PATH, null, null ); }
  ...
  void loadPreferences()
  {
    ...
    try {
      mCalibAlgo    = Integer.parseInt( prefs.getString( key[k++], CALIB_MAX_ITER ) ); // DISTOX_CALIB_MAX_IT
    } catch ( NumberFormatException e ) {
      mCalibAlgo  = Integer.parseInt( CALIB_MAX_ITER );
    }
    ...
    try {
      mLineThickness = Float.parseFloat( prefs.getString( key[k++], "1.0f" ) );  // DISTOX_LINE_THICKNESS 41
    } catch ( NumberFormatException e ) {
      mLineThickness = 1.0f;
    }
    ...
    // Log.v(TopoDroidApp.TAG, "onSharePreferenceChanged " + k );
      ...
      if ( k.equals( key[ nk++ ] ) ) { // DISTOX_HIDE_HELP
        ...
        try {
          ...
          mTextSize = Integer.parseInt( TEXT_SIZE );
        }
        ...
        try {
          mCalibAlgo    = Integer.parseInt( prefs.getString( k, CALIB_MAX_ITER ) ); // DISTOX_CALIB_MAX_IT
        } catch ( NumberFormatException e ) {
          mCalibAlgo  = Integer.parseInt( CALIB_MAX_ITER );
        }
        ...
        try {
          mHThreshold = Float.parseFloat( prefs.getString( k, "60" ) );  // DISTOX_HTHRESHOLD 38
        } catch ( NumberFormatException e ) {
          mHThreshold = 60;
        }
      } else if ( k.equals( key[ nk++ ] ) ) {
        try {
          mStationSize   = Float.parseFloat( prefs.getString( k, "24" ) ); // DISTOX_STATION_SIZE 39
        } catch ( NumberFormatException e ) {
          mStationSize = 24;
        }
        ...
        try {
          mLabelSize   = Float.parseFloat( prefs.getString( k, "24" ) ); // DISTOX_LABEL_SIZE 40
        } catch ( NumberFormatException e ) {
          mLabelSize = 24;
        }
        ...
        try {
          line_thickness = Float.parseFloat( prefs.getString( k, "1" ) );  // DISTOX_LINE_THICKNESS 41
        } catch ( NumberFormatException e ) {
          // line_thickness = 1; /* nothing */
        }
        if ( line_thickness != mLineThickness ) {
          mLineThickness = line_thickness;
  ...
  public String exportSurveyAsCsx( DrawingActivity sketch, String origin )
  {
    ...
    String cave = mySurvey.toUpperCase();
...
// ++++++++++++++++ SHOTS
      ...
      if ( fixed.size() > 0 ) {
        for ( FixedInfo fix : fixed ) {
          ...
          pw.format("       <coordinate latv=\"%.7f\" longv=\"%.7f\" altv=\"%.2f\" lat=\"%.7f N\" long=\"%.7f E\" format=\"dd.ddddddd N\" alt=\"%.2f\" />\n",
  ...
  public String exportSurveyAsTop( DrawingActivity sketch, String origin )
  {
    ...
    try {
      ...
      TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "exportSurveyAsTop date parse error " + info.date );
    }
  ...
  public String exportSurveyAsTh()
  {
    ...
    try {
      ...
      pw.format("    declination %.2f degrees\n", info.declination );
  ...
  public String exportSurveyAsSvx()
  {
    ...
    try {
      ...
      pw.format("  *calibrate declination %.2f\n", info.declination );
  ...
  public String exportSurveyAsDat()
  {
    ...
    try {
      ...
      try {
        ...
        TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "exportSurveyAsDat date parse error " + date );
      }
  ...
  public String exportCalibAsCsv()
  {
    ...
    try {
      ...
      List<CalibCBlock> list = mData.selectAllGMs( mCID, 1 ); // status 1 --> all shots
      for ( CalibCBlock b : list ) {
        b.computeBearingAndClino();
        pw.format("%d, %d, %d, %d, %d, %d, %d, %d, %.2f, %.2f, %.2f, %.4f, %d\n",
