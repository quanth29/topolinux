                        ...
                        implements DataListener
{
  ...
  String getConnectionStateTitleStr()
  {
   ...
   void start()
   {  
     // TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler start()");
     mClient = false;
     mDevice = null;
     mSyncService.start();
   }
   ...
   void stop() 
   { 
     // TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler stop()");
     stopSendThread();
     mSyncService.stop();
   }
   ...
   {
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler connect() " + device.getName() );
     if ( mDevice != device ) {
       mRun = false;
       if ( mSendThread != null ) {
         try {
   ...
   private void reconnect()
   {
     ...
     if ( ! mClient ) return;
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler reconnect() ");
   ...
   {
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler connectionFailed() ");
     if ( mClient ) {
       mClient = false;
       mDevice = null;
   ...
   {
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler disconnect() ");
   ...
   {
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler syncDevice() ");
     // if ( device.getName().equals( mDevice.getName() ) {
       doSyncCounter();
     // }
   }
   ...
   {
     // TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler write CNT " + buffer[0] + " key " + buffer[1] );
     mSyncService.write( buffer );
   }

   void startSendThread()
   {
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler startSendThread()");
     mRun = true;
   ...
   void stopSendThread()
   {
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ConnectionHandler stopSendThread()");
     mRun = false;
     if ( mSendThread != null ) {
       try {
         mSendThread.join();
   ...
   void doAcknowledge( int cnt )
   {
     ...
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "ACK write <" + cnt + ">" );
     write( mAck );
   }
   ...
   void doSyncCounter( )
   {
     ...
     mAck[2] = DataListener.EOL;
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "SYNC write <" + mSendCounter + ">" );
     write( mAck );
   }
   ...
   void onRecv( int bytes, byte[] buffer ) 
   {
     ...
     if ( key == DataListener.SYNC ) { // sync request
       ...
       synchronized( mBufferQueue ) {
         ...
         if ( item != null ) {
           ...
           TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "recv ACK <" + cnt + "> removed. queue size " + mBufferQueue.size() );
         } else {
           TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "recv ACK <" + cnt + "> not found" );
         }
       }
       return;
     }
     ...
     if ( mRecvCounter != cnt ) {
       TopoDroidApp.Log( TopoDroidApp.LOG_ERR, "recv ERROR <" + cnt + "|" + key + "> expected " + mRecvCounter );
       // should ack again ?
       // doAcknowledge( cnt );
       return;
     }
     ...
     TopoDroidApp.Log( TopoDroidApp.LOG_SYNC,
     ...
     switch ( key ) {
       ...
       case DataListener.PLOT_INSERT:
         mApp.mData.insertPlot( mSID, Integer.parseInt(data[1]), data[2], Integer.parseInt(data[3]), Integer.parseInt(data[4]),
           ...
           Double.parseDouble(data[9]), Double.parseDouble(data[10]), false ); 
         break;
  ...
  public void onUpdateSurveyInfo( long id, String date, String team, double decl, String comment ) 
  {
    ...
    pw.format( "%s|%s|%.2f|%s|", date, team, decl, comment );
  ...
  public void onUpdateShotAMDR( long sid, long id, double acc, double mag, double dip, double roll ) 
  {
    StringWriter sw = new StringWriter();
    PrintWriter pw  = new PrintWriter( sw );
    pw.format( "%d|%d|%.2f|%.2f|%.2f|%.2f|", (int)sid, (int)id, acc, mag, dip, roll );
                          ...
                          long extend, long flag, long leg, long status, String comment ) 
  {
    StringWriter sw = new StringWriter();
    PrintWriter pw  = new PrintWriter( sw );
    pw.format( "%d|%d|%s|%s|%.2f|%.1f|%.1f|%.1f|%d|%d|%d|%d|%s|", 
  ...
  public void onInsertShotAt( long sid, long at, double d, double b, double c, double r ) 
  {
    StringWriter sw = new StringWriter();
    PrintWriter pw  = new PrintWriter( sw );
    pw.format( "%d|%d||%.2f|%.1f|%.1f|%.1f|", (int)sid, (int)at, d, b, c, r );
    enqueue( DataListener.SHOT_INSERT, sw.getBuffer().toString() );
  }
  ...
  public void onInsertPlot( long sid, long id, String name, long type, long status, String start, String view,
                            double xoffset, double yoffset, double zoom, double azimuth ) 
  {
    StringWriter sw = new StringWriter();
    PrintWriter pw  = new PrintWriter( sw );
    pw.format( "%d|%d|%s|%d|%d|%s|%s|%.2f|%.2f|%.2f|%.2f|",
      (int)sid, (int)id, name, (int)type, (int)status, start, view, xoffset, yoffset, zoom, azimuth );
  ...
  private class SendThread extends Thread
  {
    ...
    public void run()
    {
        ...
        try {
          ...
          if ( mRun ) {
            ...
            if ( item != null ) {
              ...
              TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "lastByte " + lastByte + " cnt " + cnt );
              if ( cnt > 10 ) {
                // bail-out
                disconnect( mDevice );
              } else {
                cnt = 0;
                lastByte = buffer[0];
                TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "data write <" + buffer[0] + "|" + buffer[1] + ">" );
                write( item.mData );
              }
            }
      ...
      TopoDroidApp.Log( TopoDroidApp.LOG_SYNC, "SendThread exiting");
    }
  }
