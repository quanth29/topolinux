...
public class Calibration
{
  ...
  void EnforceMax2( Vector b, Matrix a )
  {
    ...
    if ( max >= 2.0 ) {
      ...
      TopoDroidApp.Log( TopoDroidApp.LOG_CALIB, "EnforceMax2 scale by " + m1 );
      b.x *= m1;
      b.y *= m1;
      b.z *= m1;
      a.x.x *= m1;
  ...
  public Calibration( int N, TopoDroidApp app0, boolean nonLinear )
  {
    num = 0;
    if ( N > 0 ) Reset( N );
    this.app = app0;
  ...
  public byte[] GetCoeff()
  {
    ...
    if ( mNonLinear ) {
      v = (long)(nL.x * TopoDroidApp.FN); if ( v <= 0 ) v = 0xff + v;
      coeff[48] = (byte)( (v-1) & 0xff );
      v = (long)(nL.y * TopoDroidApp.FN); if ( v <= 0 ) v = 0xff + v;
      coeff[49] = (byte)( (v-1) & 0xff );
      v = (long)(nL.z * TopoDroidApp.FN); if ( v <= 0 ) v = 0xff + v;
  ...
  {
    mDelta = 0.0f;
    TopoDroidApp.Log( TopoDroidApp.LOG_CALIB, "Calibrate: data nr. " + idx 
  ...
  public void Reset( int N )
  {
    ...
    aM = null;
    bM = null;
    TopoDroidApp.Log( TopoDroidApp.LOG_CALIB, "Reset calibration " + N + " data");
  }
...
/* ============================================================ */
  ...
  {
    if ( ! TopoDroidApp.LOG_CALIB ) return;
    StringWriter sw = new StringWriter();
    PrintWriter  pw = new PrintWriter( sw );
...
/* ============================================================ */
  ...
  {
    float mv = v.maxAbsValue() * TopoDroidApp.FV;
    float mm = m.maxAbsValue() * TopoDroidApp.FV;
    if ( mv > mm ) mm = mv;
    if ( mm > 256*128 ) {
      mv = 256*128 / mm;
      m.scaleBy( mv );
  ...
  {
    float max = 127.0f / TopoDroidApp.FN;
