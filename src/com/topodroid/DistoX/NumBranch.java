...
public class NumBranch 
{
  ...
  //   TopoDroidApp.Log( TopoDroidApp.LOG_NUM, sb.toString() );
  // }
  ...
  {
    // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "Br add shot(" + shot.from.name + "-" + shot.to.name + ") bdir " + shot.mBranchDir + " sdir " + shot.mDirection );
    shots.add( shot );
  ...
  void computeError()
  {
    ...
    for ( NumShot sh : shots ) {
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "Br sh " + sh.from.name + "-" + sh.to.name + " Br Err " + e + " " + s );
    }
    
    // NumStation st = ( n1 == null )? shots.get(0).from : n1.station;
    // NumStation sf = ( n2 == null )? st : n2.station;     // Log only
    // dump();
    // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "Br " + st.name + "=" + sf.name + " delta " + e + " " + s + " " + v );
  }
  ...
  void compensateError( float e0, float s0, float v0 )
  {
    ...
    // NumStation sf = ( n2 == null )? st : n2.station;     // Log only
    // dump();
    // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "Br " + st.name + "=" + sf.name + " compensate error " + e0 + " " + s0 + " " + v0 );
    ...
    for ( NumShot sh : shots ) {
      ...
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "shift shot " + sh.from.name + "-" + sh.to.name + " shift " + e1 + " " + s1 + "  " + v1 );
      // TopoDroidApp.Log( TopoDroidApp.LOG_NUM, "corrected block " + sh.from.name + "-" + sh.to.name + " " + sh.mLength + " " + sh.mBearing + " " + sh.mClino );
    }
  }
