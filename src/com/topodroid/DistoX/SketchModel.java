...
class SketchModel
{
  ...
  void makeConvexSurface()
  {
    ...
    if ( splay1.size() < 2 || splay2.size() < 2 ) {
      Log.e( TopoDroidApp.TAG, "makeConvexSurface too few spalys " + splay1.size() + " " + splay2.size() );
      return;
    }
    ...
    try {
      ...
      while ( (line = readLine(br)) != null ) {
        ...
        if ( vals[k].equals( "encoding" ) ) {
            ...
            while ( ! line.equals( "endsurface" ) && ! line.equals( "endjoin" ) ) {
              if ( line.startsWith( "vertex" ) ) { // read vertices
                ...
                while ( ! line.equals( "endvertex" ) ) {
                  ...
                  x =   Float.parseFloat( pt[1] ); // / TopoDroidApp.TO_THERION;
                  y = - Float.parseFloat( pt[2] ); // / TopoDroidApp.TO_THERION;
                  z = - Float.parseFloat( pt[3] ); // / TopoDroidApp.TO_THERION;
          ...
          x = y = z = 0.0f;
          if ( ++k < vals.length ) {
            x =   Float.parseFloat( vals[k] ); // / TopoDroidApp.TO_THERION; // th3 has east
          }
          if ( ++k < vals.length ) {
            y = - Float.parseFloat( vals[k] ); // / TopoDroidApp.TO_THERION;   // north
          }
          if ( ++k < vals.length ) {
            z = - Float.parseFloat( vals[k] ); // / TopoDroidApp.TO_THERION; // pos-Z
          }
          if ( ++k < vals.length ) {
            type = vals[k];
              ...
              if ( ptindex < DrawingBrushPaths.mPointLib.mAnyPointNr ) {
                ...
                while ( ++k < vals.length ) {
                  if ( vals[k].equals("-orientation") ) {
                    x = y = z = 0.0f;
                    if ( ++k < vals.length ) {
                      x =   Float.parseFloat( vals[k] ); // / TopoDroidApp.TO_THERION; 
                    }
                    if ( ++k < vals.length ) {
                      y = - Float.parseFloat( vals[k] ); // / TopoDroidApp.TO_THERION;
                    }
                    if ( ++k < vals.length ) {
                      z = - Float.parseFloat( vals[k] ); // / TopoDroidApp.TO_THERION;
                    }
          ...
          if ( ++k < vals.length ) {
            ...
            if ( ! ( line.equals( "endline" ) || line.equals( "endarea" ) ) ) { 
              ...
              z = - Float.parseFloat( pt[2] ); // / TopoDroidApp.TO_THERION;
              ...
              while ( (line = readLine( br )) != null ) {
                ...
                if ( path != null ) {
                  ...
                  if ( vals2.length == 3 ) {
                    x  =   Float.parseFloat( vals2[0] ); // / TopoDroidApp.TO_THERION;
                    y  = - Float.parseFloat( vals2[1] ); // / TopoDroidApp.TO_THERION;
                    z  = - Float.parseFloat( vals2[2] ); // / TopoDroidApp.TO_THERION;
                    path.addLinePoint( x, y, z );
                  } else if ( vals2.length == 9 ) {
                    x1 =   Float.parseFloat( vals2[0] ); // / TopoDroidApp.TO_THERION;
                    y1 = - Float.parseFloat( vals2[1] ); // / TopoDroidApp.TO_THERION;
                    z1 = - Float.parseFloat( vals2[2] ); // / TopoDroidApp.TO_THERION;
                    x2 =   Float.parseFloat( vals2[3] ); // / TopoDroidApp.TO_THERION;
                    y2 = - Float.parseFloat( vals2[4] ); // / TopoDroidApp.TO_THERION;
                    z2 = - Float.parseFloat( vals2[5] ); // / TopoDroidApp.TO_THERION;
                    x  =   Float.parseFloat( vals2[6] ); // / TopoDroidApp.TO_THERION;
                    y  = - Float.parseFloat( vals2[7] ); // / TopoDroidApp.TO_THERION;
                    z  = - Float.parseFloat( vals2[8] ); // / TopoDroidApp.TO_THERION;
