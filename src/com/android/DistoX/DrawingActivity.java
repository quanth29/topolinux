                                      ...
                                      , ILister
{
  ...
  void setButton3( int type )
  {
    ...
    public void onCreate(Bundle savedInstanceState) 
    {
      ...
      int type     = (int)extras.getLong( mApp.TOPODROID_PLOT_TYPE );
    ...
    private void loadFiles( )
    {
      ...
      mPid = mPid1;
      // Log.v( TopoDroidApp.TAG, "load pid " + mPid1 + " " + mPid2 );
   ...
   private void resetReference( PlotInfo plot )
   {
    ...
    private void doSelectAt( float x_scene, float y_scene )
    {
      ...
      if ( mMode == MODE_EDIT ) {
        SelectionSet selection = mDrawingSurface.getItemsAt( x_scene, y_scene );
    ...
    {
      mDrawingSurface.eraseAt( x_scene, y_scene );
    }

    void updateBlockName( DistoXDBlock block, String from, String to )
    {
      if ( mFullName2 == null ) return; // nothing for PLOT_SECTION
              ...
              if ( mPointCnt > mApp.mLineType ) {
                if ( ! ( mSymbol == SYMBOL_LINE && mCurrentLinePath.mLineType == DrawingBrushPaths.mLineLib.mLineSectionIndex ) 
                  ...
                  if ( mSymbol == SYMBOL_LINE ) {
                    ...
                    if ( mCurrentLinePath.mLineType == DrawingBrushPaths.mLineLib.mLineSectionIndex ) {
                      // start section dialoq
                      new DrawingLineSectionDialog( this, mCurrentLinePath ).show();
                    }
                  } else { //  mSymbol == SYMBOL_AREA
          ...
          switch ( sp.type() ) {
            ...
            case DrawingPath.DRAWING_PATH_LINE:
              ...
              if ( line.mLineType == DrawingBrushPaths.mLineLib.mLineSectionIndex ) {
                new DrawingLineSectionDialog( this, line ).show();
              } else {
    ...
    void makeSectionDraw( DrawingLinePath line, String id )
    {
      mCurrentLine = DrawingBrushPaths.mLineLib.mLineWallIndex;
      ...
      if ( pid < 0 ) { 
        pid = mApp.mData.insertPlot( mApp.mSID, -1L, mSectionName, PlotInfo.PLOT_SECTION, 0L,
                                         "-1", "-1", 0, 0, TopoDroidApp.mScaleFactor );
      }
      ...
      if ( pid >= 0 ) {
        ...
        drawIntent.putExtra( TopoDroidApp.TOPODROID_PLOT_TYPE, PlotInfo.PLOT_SECTION );
