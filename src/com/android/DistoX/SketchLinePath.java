...
public class SketchLinePath extends SketchPath
{
   ...
   */
  public SketchLinePath( int path_type, int th_type, int view_type, String s1, String s2, SketchPainter painter )
  {
  ...
  void addLinePoint( float x, float y, float z ) 
  {
  ...
  {
    // Log.v("DistoX", "Line " + mViewType + " toTherion, stations " + st1 + " " + st2 );
    StringWriter sw = new StringWriter();
    PrintWriter pw  = new PrintWriter(sw);
    if ( mType == DrawingPath.DRAWING_PATH_LINE ) {
      pw.format("line %s %s -shot %s %s", SketchDef.view_type[ mViewType ], 
                                          DrawingBrushPaths.getLineThName(mThType), st1, st2 );
      if ( mClosed ) {
        pw.format(" -close on");
      }
    } else if ( mType == DrawingPath.DRAWING_PATH_AREA ) {
      // area border is closed by default
      pw.format("area %s %s -shot %s %s", SketchDef.view_type[ mViewType ],
                                          DrawingBrushPaths.getAreaThName(mThType), st1, st2 );
    }
