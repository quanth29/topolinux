                           ...
                           implements View.OnClickListener
{
  ...
  private void setup()
  {
    ...
    for (int k = 0; k<19; ++k ){
      float clino = clino_angles[k] * TopoDroidApp.GRAD2RAD_FACTOR;
      for (int j=t_offset[k]; j<t_offset[k]+t_size[k]; ++j ) {
        angles[j] = new Direction(
                      TopoDroidApp.M_PI + ( TopoDroidApp.M_2PI * (j - t_offset[k]) ) / t_size[k],
                      clino, 
                      1.0f );
      }
    }
  ...
  private void evalCoverage( List<CalibCBlock> clist, Calibration transform )
  {
    ...
    for ( CalibCBlock b : clist ) {
      ...
      float compass = b.mBearing * TopoDroidApp.GRAD2RAD_FACTOR;
      float clino   = b.mClino   * TopoDroidApp.GRAD2RAD_FACTOR;
      if ( b.mGroup == old_grp ) {
        if ( cnt_avg > 0 && Math.abs( compass - compass_avg / cnt_avg ) > 1.5f * TopoDroidApp.M_PI ) {
          if ( compass > TopoDroidApp.M_PI ) {
            compass -= TopoDroidApp.M_2PI; // average around 0
          } else {
            compass += TopoDroidApp.M_2PI; // average around 360
          }
        }
        clino_avg   += clino;
        compass_avg += compass;
