...
public class NumBranch 
{
  ...
  {
    // Log.v( TopoDroidApp.TAG, "Br add shot(" + shot.from.name + "-" + shot.to.name + ") bdir " + shot.mBranchDir + " sdir " + shot.mDirection );
    shots.add( shot );
    // float d = shot.block.mLength;
    // len += d;
  }
  ...
  void computeError()
  {
    ...
    for ( NumShot sh : shots ) {
      float d = sh.block.mLength;
      float b = sh.block.mBearing * grad2rad;
      float c = sh.block.mClino * grad2rad;
      len += d;
      ...
      // Log.v( TopoDroidApp.TAG, "Br sh " + sh.from.name + "-" + sh.to.name + " Br Err " + e + " " + s );
    }
    
    // NumStation st = ( n1 == null )? shots.get(0).from : n1.station;
    // NumStation sf = ( n2 == null )? st : n2.station;     // Log only
    // dump();
    // Log.v( TopoDroidApp.TAG, "Br " + st.name + "=" + sf.name + " delta " + e + " " + s + " " + v );
  }
  ...
  void compensateError( float e0, float s0, float v0 )
  {
    ...
    // NumStation sf = ( n2 == null )? st : n2.station;     // Log only
    // dump();
    // Log.v( TopoDroidApp.TAG, "Br " + st.name + "=" + sf.name + " compensate error " + e0 + " " + s0 + " " + v0 );
    ...
    for ( NumShot sh : shots ) {
      ...
      sh.block.mBearing = (float)(Math.atan2( e1, -s1 ) / grad2rad); // + 90.0f * (1 - sh.mDirection);
      if ( sh.block.mBearing >= 360.0f) sh.block.mBearing -= 360.0f;
      sh.block.mClino   = (float)(Math.atan2( -v1, h1 ) / grad2rad); // * sh.mDirection;
      sh.block.mLength  = (float)(Math.sqrt( h1*h1 + v1*v1 ));
      // Log.v( TopoDroidApp.TAG, "shift shot " + sh.from.name + "-" + sh.to.name + " shift " + e1 + " " + s1 + "  " + v1 );
      // Log.v( TopoDroidApp.TAG, "corrected block " + sh.from.name + "-" + sh.to.name + " " + sh.block.mLength + " " + sh.block.mBearing + " " + sh.block.mClino );
    }
  }
